...
export default function Table({ data, columns, onEdit, onDelete, rowKey, rowClickRoute, rowActions, showEditDelete }: TableProps) {
    ...
    const columnsToRender = columns && columns.length > 0
        ? columns
        : tableData.length > 0
            ? Object.keys(tableData[0]).filter(col => {
                ...
                if (rowKey !== "userid" && (lowerCol === "userid" || lowerCol === "users")) return false;
    ...
    const handleDelete = async () => {
        ...
        try {
            ...
            if (result.success) {
                ...
                setTableData(prev => prev.filter(u => u[rowKey] !== selectedData[rowKey]));
            } else {
                        ...
                        {tableData.map((row, idx) => (
                            ...
                            >
                                {columnsToRender.map((col, i) => {
                                    const value = row[col];
                                    if (col.toLowerCase() === "is_request_title_disable") {
                                        return (
                                            <td key={i} className="px-6 py-4 whitespace-nowrap text-sm">
                                                <span className={`px-3 py-1 rounded-full text-white text-xs font-semibold ${value ? "bg-red-500" : "bg-green-500"}`}>
                                    ...
                                    if (col.toLowerCase() === "roles" && value) {
                                        return (
                                            <td key={i} className="px-6 py-4 whitespace-nowrap text-sm">
                                                <span
                                                    className={`px-2 py-1 rounded-full text-xs font-semibold ${value === "Admin"
                                    ...
                                    if (col.toLowerCase() === "status") {
                                        ...
                                        return (
                                            <td key={i} className="px-3 py-2 whitespace-nowrap text-sm">
                                                <span className={`px-3 py-1 rounded-full font-semibold border ${bgColor}`}>
                                                    {value}
                                                </span>
                                            </td>
                                        );
                                    ...
                                    if (col.toLowerCase() === "priority") {
                                        ...
                                        if (value?.toLowerCase() === "high") bgColor = "bg-red-500 text-white";
                                        else if (value?.toLowerCase() === "medium") bgColor = "bg-yellow-500 text-white";
                                        else if (value?.toLowerCase() === "low") bgColor = "bg-green-500 text-white";

                                        return (
                                            <td key={i} className="px-6 py-4 whitespace-nowrap text-sm">
                                                <span className={`px-3 py-1 rounded-full font-semibold ${bgColor}`}>
                                                    {value}
                                                </span>
                                            </td>
                                        );
                                    }
                                    if (col.toLowerCase() === "created" || col.toLowerCase() === "created_at" || col.toLowerCase() === "modified" || col.toLowerCase() === "date") {
                                        return (
                                ...
                                {rowActions?.map((action, idx) => (
                                    <td key={idx} className="px-3 py-2">{action.render(row)}</td>
                                ))}
                                    ...
                                    <>
                                        <td className="px-6 py-4 whitespace-nowrap text-sm">
                                            <button
                                                ...
                                                className="bg-blue-500 hover:bg-blue-600 text-white px-3 py-1 rounded-md font-medium transition-colors cursor-pointer duration-200 focus:outline-none focus:ring-2 focus:ring-blue-400"
                                            >
                                                Edit
                                            </button>
                                        </td>
                                        <td className="px-6 py-4 whitespace-nowrap text-sm">
                                            <button
                                                ...
                                                disabled={isDeleting}
                                                className={`bg-red-500 hover:bg-red-600 text-white px-4 py-1 rounded-md font-medium transition-colors cursor-pointer duration-200 focus:outline-none focus:ring-2 focus:ring-red-400 ${isDeleting ? "opacity-50 cursor-not-allowed" : ""}`}
                                            >
                                                Delete
                                            </button>
                                        </td>
