...
const actionMap: Record<DecisionModalProps["action"], ActionHandler | undefined> = {
...
export default function DecisionModal({
    ...
    const availableTechnicians = isReassign
        ? technicians.filter(
            tech => tech.id !== request.assigned_to_userid
        )
        : technicians;
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center">
            ...
            >
                ...
                <div className="border rounded-lg p-4 mb-4 bg-gray-50 space-y-2">
                    ...
                    <p><strong>Submitted By:</strong> {request.userfullname}</p>
                    <p><strong>Priority:</strong> {request.priority}</p>
                    <p><strong>Requested On:</strong> {formatDate(request.created_at)}</p>
                    {action === "Reassign" && (
                        <div className="mb-4">
                            <p>
                                <strong>Currently Assigned To:</strong> {request.assigned_to || "N/A"}
                            </p>
                        </div>
                    )}
                </div>
                ...
                <div className="mb-2">
                    Comment {action === "Close" || action === "Decline" ? "" : "(optional)"}
                    <textarea
                        className="w-full border rounded-lg p-2"
                        rows={3}
                        value={comment}
                ...
                <div className="flex justify-end gap-3">
                    ...
                    <button
                        className={`px-4 py-2 rounded-lg text-white transition ${action === "Approve"
                            ? "bg-green-600 hover:bg-green-700"
                            : action === "Reassign"
                                ? "bg-indigo-600 hover:bg-indigo-700"
                                : action === "Close"
                                    ? "bg-gray-600 hover:bg-gray-700"
                                    : "bg-red-600 hover:bg-red-700" // Decline
                            }`}
